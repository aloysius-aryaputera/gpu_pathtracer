...
__global__ void create_point_image(
  vec3 *fb, Camera **camera, PPMHitPoint** point_array, int num_pts
);
...
__global__ void create_point_image(
  ...
  if (point_array[idx] -> location.vector_is_inf()) return;
...
__global__ void create_point_image(
  ...
  if (
    ...
    fb[pixel_index] = point_array[idx] -> filter;
  }
}
